...
class FstImpl {
 public:
  ...
  FstImpl &operator=(const FstImpl &impl) {
    ...
    type_ = impl.type_;
    isymbols_ = impl.isymbols_
                  ? std::unique_ptr<SymbolTable>{impl.isymbols_->Copy()}
                  : nullptr;
    osymbols_ = impl.osymbols_
                  ? std::unique_ptr<SymbolTable>{impl.osymbols_->Copy()}
                  : nullptr;
    return *this;
  }
...
template<class Arc>
std::string FstToString(const Fst<Arc> &fst, const FstWriteOptions &options =
