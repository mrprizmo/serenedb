...
namespace boost { namespace text {
    ...
    private:
            ...
            noexcept(!throw_on_error)
        {
            ...
            // Because his table implementation really sucks, better to remove it at all
#if 1
            /*
                Unicode 3.9/D92
            ...
            */
            ...
            if (detail::in(0xc2, curr_c, 0xdf)) {
                ...
                ++next;
                // Three-byte
            } else if (detail::in(0xe0, curr_c, 0xed)) {
                value = curr_c & 0b00001111;
                ++next;
                if (at_end(next))
                    return get_value_result{replacement_character(), next};
                curr_c = *next;
                if (!check_continuation(curr_c, curr_c == 0xe0 ? 0xa0 : 0x80, curr_c == 0xed ? 0x9f : 0xbf))
                    return get_value_result{replacement_character(), next};
                value = (value << 6) + (curr_c & 0b00111111);
                ++next;
                if (at_end(next))
                ...
                ++next;
                // Four-byte
            } else if (detail::in(0xf0, curr_c, 0xf4)) {
                value = curr_c & 0b00000111;
                ++next;
                if (at_end(next))
                    return get_value_result{replacement_character(), next};
                curr_c = *next;
                if (!check_continuation(curr_c, curr_c == 0xf0 ? 0x90 : 0x80, curr_c == 0xf4 ? 0x8f : 0xbf))
                    return get_value_result{replacement_character(), next};
                value = (value << 6) + (curr_c & 0b00111111);
                ++next;
                if (at_end(next))
