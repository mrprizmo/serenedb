...
inline size_t JSONStringCopyC(uint8_t* dst, const uint8_t* src, size_t limit) {
...
inline size_t JSONStringCopyCheckUtf8C(uint8_t* dst, const uint8_t* src,
                                       size_t limit) {
  // Copy up to limit uint8_t from src to dst.
  // Stop at the first control character or backslash or double quote.
...
inline size_t JSONSkipWhiteSpaceC(const uint8_t* src, size_t limit) {
...
inline bool ValidateUtf8StringC(const uint8_t* src, size_t limit) {
  return Utf8Helper::isValidUtf8(src, static_cast<ValueLength>(limit));
}

#if defined(__SSE4_2__) && VPACK_ASM_OPTIMIZATIONS == 1
...
size_t JSONStringCopySSE42(uint8_t* dst, const uint8_t* src, size_t limit) {
  alignas(16) static const char kRanges[17] =
    "\x20\x21\x23\x5b\x5d\xff          ";
  //= "\x01\x1f\"\"\\\\\"\"\"\"\"\"\"\"\"\"";

  const __m128i r = _mm_load_si128(reinterpret_cast<const __m128i*>(kRanges));
  size_t count = 0;
  int x = 0;
  while (limit >= 16) {
    const __m128i s = _mm_loadu_si128(reinterpret_cast<const __m128i*>(src));
...
size_t JSONStringCopyCheckUtf8SSE42(uint8_t* dst, const uint8_t* src,
                                    size_t limit) {
  alignas(16) static const unsigned char kRanges[17] =
...
size_t JSONSkipWhiteSpaceSSE42(const uint8_t* ptr, size_t limit) {
  alignas(16) static const char kWhite[17] = " \t\n\r            ";
  const __m128i w = _mm_load_si128(reinterpret_cast<const __m128i*>(kWhite));
  size_t count = 0;
  int x = 0;
  while (limit >= 16) {
    const __m128i s = _mm_loadu_si128(reinterpret_cast<const __m128i*>(ptr));
...
bool ValidateUtf8StringSSE42(uint8_t const* src, size_t len) {
  if (len >= 16) {
    return ValidateUtf8FastSse42(src, len);
  }
  return Utf8Helper::isValidUtf8(src, len);
}

#endif
#if defined(__AVX2__) && VPACK_ASM_OPTIMIZATIONS == 1
...
bool ValidateUtf8StringAVX(const uint8_t* src, size_t len) {
  if (len >= 32) {
    return ValidateUtf8FastAvx(src, len);
  }
  return Utf8Helper::isValidUtf8(src, len);
}
...
[[maybe_unused]] const EnableNative kInit;

}  // namespace

size_t (*gJsonStringCopy)(uint8_t*, const uint8_t*, size_t) = JSONStringCopyC;

size_t (*gJsonStringCopyCheckUtf8)(uint8_t*, const uint8_t*,
                                   size_t) = JSONStringCopyCheckUtf8C;

size_t (*gJsonSkipWhiteSpace)(const uint8_t*, size_t) = JSONSkipWhiteSpaceC;

bool (*gValidateUtf8String)(const uint8_t*, size_t) = ValidateUtf8StringC;

void EnableNativeStringFunctions() noexcept {
  EnableBuiltinStringFunctions();
#if defined(__SSE4_2__) && VPACK_ASM_OPTIMIZATIONS == 1
  if (HasSsE42()) {
    gJsonStringCopy = JSONStringCopySSE42;
    gJsonStringCopyCheckUtf8 = JSONStringCopyCheckUtf8SSE42;
    gJsonSkipWhiteSpace = JSONSkipWhiteSpaceSSE42;
    gValidateUtf8String = ValidateUtf8StringSSE42;
  }
#elif defined(__aarch64__) && VPACK_ASM_OPTIMIZATIONS == 1
  gValidateUtf8String = ValidateUtf8StringSSE42;
#endif
#if defined(__AVX2__) && VPACK_ASM_OPTIMIZATIONS == 1
  if (HasAvX2()) {
    gValidateUtf8String = ValidateUtf8StringAVX;
  }
#endif
}

void EnableBuiltinStringFunctions() noexcept {
  gJsonStringCopy = JSONStringCopyC;
  gJsonStringCopyCheckUtf8 = JSONStringCopyCheckUtf8C;
  gJsonSkipWhiteSpace = JSONSkipWhiteSpaceC;
  gValidateUtf8String = ValidateUtf8StringC;
}

}  // namespace vpack

#if defined(COMPILE_VPACK_ASM_UNITTESTS)
...
void TestStringCopyCorrectness(uint8_t* src, uint8_t* dst, size_t size) {
  ...
  for (size_t salign = 0; salign < 16; salign++) {
    ...
    for (size_t dalign = 0; dalign < 16; dalign++) {
      dst += dalign;
      for (size_t i = 0;
           i < static_cast<int>(sizeof(gTestPositions) / sizeof(int)); i++) {
        uint8_t merk;
        int off = gTestPositions[i];
        size_t pos;
        if (off >= 0) {
          pos = off;
        } else {
        ...
        merk = src[pos];
        src[pos] = '"';
        copied = gJsonStringCopy(dst, src, size);
  ...
  std::cout << "Tests took altogether " << total_time.count() << " seconds."
            << std::endl;
}
  ...
  for (int salign = 0; salign < 16; salign++) {
    ...
    for (int dalign = 0; dalign < 16; dalign++) {
      ...
      for (int i = 0;
        ...
        merk = src[pos];
        src[pos] = '"';
        copied = gJsonStringCopyCheckUtf8(dst, src, size);
        if (copied != pos || memcmp(dst, src, copied) != 0) {
          std::cout << "Error: " << salign << " " << dalign << " " << i << " "
                    << pos << " " << copied << std::endl;
        }
...
void TestSkipWhiteSpaceCorrectness(uint8_t* src, size_t size) {
  ...
  for (int salign = 0; salign < 16; salign++) {
    src += salign;
    for (int i = 0; i < static_cast<int>(sizeof(gTestPositions) / sizeof(int));
         i++) {
      uint8_t merk;
      int off = gTestPositions[i];
      size_t pos;
      ...
      merk = src[pos];
      src[pos] = 'x';
      copied = gJsonSkipWhiteSpace(src, size);
      if (copied != pos) {
  ...
  std::chrono::duration<double> total_time =
    std::chrono::duration_cast<std::chrono::duration<double>>(now - start);
  std::cout << "Whitespace tests took altogether " << total_time.count()
            << " seconds." << std::endl;
}
...
void RaceStringCopy(uint8_t* dst, uint8_t* src, size_t size, int repeat,
  ...
  std::cout << "Race took altogether " << total_time.count() << " seconds."
            << std::endl;
  std::cout << "Time to copy string of length " << size
            << " on average is: " << total_time.count() / repeat << "."
            << std::endl;
  std::cout << "Bytes copied per second: "
            << (double)size * (double)repeat / total_time.count() << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  for (size_t i = size + 16; i < size + 16 + 17; i++) {
    src[i] = 0;
  }
  ...
  static const char kWhitetab[17] = "       \t   \n   \r";
  for (size_t i = 0; i < size + 16; i++) {
    src[i] = kWhitetab[i % 16];
  }
  src[size + 16] = 0;
