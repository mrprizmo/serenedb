VPack - a fast and compact format for serialization and storage
============================================================================
...
JSON), Apache Thrift till Google's protocol buffers and SereneDB's
shaped JSON.
...
We have invented VPack because we need a binary format that
  ...
  - is compact
  - is largely platform independent (see [Portability](VPack.md#Portability))
...
respect to the above list. To name but a few:
  ...
  - Our own shaped JSON (used in SereneDB as internal storage format)
...
VPack format*. The library is written in C++20 so it should compile
on many up-to-date systems.

Specification
-------------

See the file [vpack.md](vpack.md) for a detailed description of
the VPack format.
