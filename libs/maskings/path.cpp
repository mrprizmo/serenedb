...
ParseResult<Path> Path::parse(std::string_view def) {
  if (def.empty()) {
    return ParseResult<Path>(ParseResult<Path>::kIllegalParameter,
  ...
  const uint8_t* p = reinterpret_cast<const uint8_t*>(def.data());
  int32_t off = 0;
  ...
  while (off < len) {
    ...
    if (ch < 0) {
      return ParseResult<Path>(
        ParseResult<Path>::kIllegalParameter,
...
bool Path::match(const std::vector<std::string_view>& path) const {
