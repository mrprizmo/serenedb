...
CpuUsageSnapshot CpuUsageSnapshot::fromString(const char* buffer,
                                              size_t buffer_size) noexcept {
  auto read_number = [](const char*& p, const char* e, bool& valid) {
    if (p >= e) {
      valid = false;
    }
    ...
    return sdb::number_utils::AtoiPositive<uint64_t>(s, p, valid);
  };

  const char* s = buffer;
  const char* e = s + buffer_size;

  bool valid = true;

  CpuUsageSnapshot snap;

  snap.user = read_number(s, e, valid);
  snap.nice = read_number(s, e, valid);
  snap.system = read_number(s, e, valid);
  snap.idle = read_number(s, e, valid);
  snap.iowait = read_number(s, e, valid);
  snap.irq = read_number(s, e, valid);
  snap.softirq = read_number(s, e, valid);
  snap.steal = read_number(s, e, valid);
  snap.guest = read_number(s, e, valid);
  snap.guestnice = read_number(s, e, valid);
...
void CpuUsageSnapshot::subtract(const CpuUsageSnapshot& other) noexcept {
