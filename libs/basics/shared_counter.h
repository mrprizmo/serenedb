...
#include "basics/shared_atomic.h"
...
struct SharedCounter {
  ...
  static uint64_t DefaultIdFunc() {
    return wyhash64(Thread::currentThreadNumber(), 0xdeadbeefdeadbeefULL);
  }
  ...
  explicit SharedCounter(const SharedCounter<stripes>& other) { copy(other); }

  SharedCounter<stripes>& operator=(const SharedCounter<stripes>& other) {
    copy(other);
    return *this;
  }
  ...
  void sub(int64_t arg, std::memory_order order = std::memory_order_seq_cst) {
    ...
    SDB_ASSERT(!everywhereNonNegative || (prev - arg >= 0));
  }
 ...
 private:
  ...
  void copy(const SharedCounter<stripes>& other) {
    if (this != &other) {
      _id = other._id;
      _mask = other._mask;
      for (size_t i = 0; i < stripes; i++) {
