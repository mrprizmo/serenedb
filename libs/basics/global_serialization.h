...
// global synchronization points must only be in SDB_IF_FAILURE blocks.
...
// all `serened` instances share a common file system and use the same
...
// `serened` instances have access. It is called `globalSLP` and resides
...
//
//  - this only works in **local** clusters, in which all serened instances
...
#else
inline void WaitForGlobalEvent(std::string_view, std::string_view) {}
inline void ObserveGlobalEvent(std::string_view, std::string_view) {}
#endif
