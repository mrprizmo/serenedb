...
struct Helpers {
  /// calculate capacity for the container for at least one more
  /// element.
  /// if this would exceed the container's capacity, use a growth factor of
  /// 1.5 to calculate the new capacity.
  template<typename T>
  static size_t nextCapacity(const T& container, size_t initial_capacity) {
    size_t capacity;
    if (container.empty()) {
      // reserve some initial space
      capacity = std::max(size_t(1), initial_capacity);
    } else {
      SDB_ASSERT(container.capacity() > 0);
      ...
      if (capacity > container.capacity()) {
        ...
        constexpr size_t kJemallocMinInPlaceExpandable = 4096;
        if (container.capacity() <
            kJemallocMinInPlaceExpandable /
    ...
    SDB_ASSERT(capacity > container.size());
    return capacity;
  }

  /// reserve space for at least one more element in the container.
  /// if this would exceed the container's capacity, use a growth factor of
  /// 1.5 to grow the container's memory.
  template<typename T>
  static void reserveSpace(T& container, size_t initial_capacity) {
    size_t capacity = nextCapacity(container, initial_capacity);
