...
/// simple struct to store a single CPU usage snapshot.
...
struct CpuUsageSnapshot {
  /// create a CpuUsageSnapshot from the contents of /proc/stat
  /// expects an input buffer with 10 numbers, each seperated by a space
  /// characters
  static CpuUsageSnapshot fromString(const char* buffer,
                                     size_t buffer_size) noexcept;

  /// subtract the values of another CpuUsageSnapshot from ourselves
  void subtract(const CpuUsageSnapshot& other) noexcept;

  /// whether or not the snapshot contains valid data.
  /// this is false for empty snapshots
  bool valid() const noexcept;

  /// clear/invalidate a snapshot
  void clear() noexcept;

  /// return total CPU time spent in the snapshot, including idle time
  uint64_t total() const noexcept;

  /// percent of user time (plus nice time) in ratio to total CPU time
  double userPercent() const noexcept {
    uint64_t t = total();
    return (t > 0) ? (100.0 * double(user + nice) / double(t)) : 0.0;
  }

  /// percent of system time in ratio to total CPU time
  double systemPercent() const noexcept {
    uint64_t t = total();
    return (t > 0) ? (100.0 * double(system) / double(t)) : 0.0;
  }

  /// percent of idle time in ratio to total CPU time
  double idlePercent() const noexcept {
    uint64_t t = total();
    return (t > 0) ? (100.0 * double(idle) / double(t)) : 0.0;
  }

  /// percent of io wait time in ratio to total CPU time
