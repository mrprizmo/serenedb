...
// metrics "section".
enum class MetricsSection : uint8_t {
  None = 0,
...
struct MetricsParts {
  MetricsParts() noexcept
    : sections(std::to_underlying(MetricsSection::None)) {}
  explicit MetricsParts(MetricsSection section) noexcept
    : sections(std::to_underlying(section)) {}
  ...
  bool includeStandardMetrics() const noexcept {
    return sections & std::to_underlying(MetricsSection::Standard);
  }

  bool includeDynamicMetrics() const noexcept {
    return sections & std::to_underlying(MetricsSection::Dynamic);
  }

  std::underlying_type_t<MetricsSection> sections;
};
